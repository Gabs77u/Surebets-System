...
def client():
    ...
    with patch("backend.apps.admin_api.DatabaseManager") as mock_db:
        ...
        db_instance.fetch_one.side_effect = lambda query, params=None: (
            ...
            if params and params[0] in ["admin", "operator", "viewer"]
            else None
        )
...
class TestAuthFlow:
    ...
    def test_login_with_cookie(self, client):
        ...
        pytest.skip(
            "Fluxo de autenticação com cookie não é mais suportado na arquitetura atual (JWT puro)"
        )
    ...
    def test_login_operator_success(self, client):
        ...
        response = client.post(
            "/api/auth/login", json={"username": "operator", "password": "password123"}
        )
    ...
    def test_login_failed(self, client):
        ...
        with patch("backend.core.auth.AuthManager.verify_password", return_value=False):
            response = client.post(
                "/api/auth/login", json={"username": "fake", "password": "wrongpass"}
            )
    ...
    def test_protected_routes_by_role(self, client):
        ...
        login = lambda u: json.loads(
            client.post(
                "/api/auth/login", json={"username": u, "password": "password123"}
            ).data
        )["access_token"]
        ...
        user_dashboard_admin = client.get(
            "/api/user/dashboard", headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert user_dashboard_admin.status_code == 200
        user_dashboard_operator = client.get(
            "/api/user/dashboard", headers={"Authorization": f"Bearer {operator_token}"}
        )
        assert user_dashboard_operator.status_code == 200
        user_dashboard_viewer = client.get(
            "/api/user/dashboard", headers={"Authorization": f"Bearer {viewer_token}"}
        )
    ...
    def test_logout_with_cookie(self, client):
        ...
        pytest.skip(
            "Fluxo de logout com cookie não é mais suportado na arquitetura atual (JWT puro)"
        )
    ...
    def test_permission_based_access(self, client):
        ...
        with patch(
            "backend.core.auth.AuthManager.has_permission",
            side_effect=lambda role, perm: role == ROLE_ADMIN
            and perm == "can_manage_users",
        ):
            # Login como admin
            ...
            admin_response = client.get(
                "/test/permission", headers={"Authorization": f"Bearer {admin_token}"}
            )
            assert admin_response.status_code == 200
    ...
    def test_roles_endpoint(self, client):
        ...
        roles_response = client.get(
            "/api/auth/roles", headers={"Authorization": f"Bearer {admin_token}"}
        )
