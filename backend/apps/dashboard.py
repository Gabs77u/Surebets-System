...
logging.basicConfig(
    level=(
        logging.INFO
        if not os.getenv("DEBUG", str(settings.DEBUG)).lower() in ["1", "true", "yes"]
        else logging.DEBUG
    ),
...
BOOKMAKERS = [{"label": name.title(), "value": name} for name in get_bookmaker_names()]
    ...
    return dbc.Card(
        [
            dbc.CardHeader(
                [
                    html.H5(
                        i18n.t("real_time_opportunities"), className="mb-0 d-inline"
                    ),
                    dbc.Button(
                        i18n.t("update_now"),
                        id="manual-refresh",
                        ...
                        [
                            dbc.Col(
                                [
                                    ...
                                    html.P(
                                        "Oportunidades Ativas", className="text-muted"
                                    ),
                                ],
                                md=3,
                            ),
                            dbc.Col(
                                [
                                    html.H4(
                                        "0%", id="avg-profit", className="text-info"
                                    ),
        ...
        [
            ...
            dbc.CardBody(
                [
                    dbc.Row(
                        [
                            dbc.Col([dcc.Graph(id="profit-distribution-chart")], md=6),
                            dbc.Col([dcc.Graph(id="bookmaker-comparison-chart")], md=6),
                        ]
                    )
                ]
            ),
...
def update_opportunities_table(
    n_intervals, n_clicks, sports, min_profit, bookmakers, search
):
    ...
    try:
        ...
        avg_profit = sum(
            float(op["profit"].replace("%", "")) for op in all_opportunities
        ) / max(total_ops, 1)
...
@app.callback(
    [Output("live-games-table", "data"), Output("upcoming-games-table", "data")],
...
def update_charts(opportunities_data):
    ...
    try:
        ...
        bm_fig = go.Figure(
            data=[
                go.Bar(
                    x=list(bookmaker_counts.keys()), y=list(bookmaker_counts.values())
                )
            ]
        )
        ...
        empty_fig.update_layout(
            title="Erro ao carregar gráficos", template="plotly_dark"
        )
        return empty_fig, empty_fig
...
@app.callback(
    [Output("session", "data"), Output("admin-login-status", "children")],
    [Input("admin-login-btn", "n_clicks")],
    [State("admin-password", "value"), State("session", "data")],
...
def admin_login(n_clicks, password, session_data):
    ...
    return session_data or {}, (
        dbc.Alert("Senha incorreta.", color="danger") if n_clicks else ""
    )


if __name__ == "__main__":
    logger.info("Iniciando dashboard consolidado do Surebets Hunter Pro")
    app.run_server(debug=settings.DEBUG, host="0.0.0.0", port=settings.PORT)
