...
class TestDatabasePerformance:
    ...
    def test_bulk_insert_performance(
        self, clean_database, benchmark_timer, memory_profiler
    ):
        ...
        for batch_size in batch_sizes:
            ...
            results[batch_size] = {"elapsed_ms": elapsed_ms, "throughput": throughput}
        ...
        for batch_size, metrics in results.items():
            ...
            logging.info(
                f"Batch {batch_size}: {metrics['elapsed_ms']:.2f}ms, {metrics['throughput']:.2f} records/sec"
            )
    ...
    def test_complex_query_performance(self, populated_database, benchmark_timer):
        ...
        logging.info(
            f"Complex query: avg={avg_time:.2f}ms, min={min_time:.2f}ms, max={max_time:.2f}ms"
        )
    ...
    def test_concurrent_read_performance(self, threading_database):
        ...
        logging.info(
            f"Concurrent reads: {num_workers} workers, avg={avg_total_time:.2f}ms, max={max_total_time:.2f}ms"
        )
    ...
    def test_mixed_workload_performance(self, threading_database):
        ...
        logging.info(
            f"Mixed workload: reads={avg_read_time:.2f}ms, writes={avg_write_time:.2f}ms"
        )
...
class TestArbitragePerformance:
    ...
    def test_arbitrage_detection_speed(self, populated_database, benchmark_timer):
        ...
        logging.info(
            f"Arbitrage detection: avg={avg_detection_time:.2f}ms, max={max_detection_time:.2f}ms"
        )
    ...
    def test_stake_calculation_performance(self, populated_database, benchmark_timer):
        ...
        if calculation_times:
            ...
            logging.info(
                f"Stake calculation: avg={avg_calc_time:.3f}ms per opportunity"
            )
...
class TestScalabilityTests:
    ...
    def test_large_dataset_performance(self, clean_database, benchmark_timer):
        ...
        # Bookmakers
        bookmaker_data = [
            (f"Bookmaker {i}", f"https://api{i}.com", True) for i in range(100)
        ]
        db.execute_many(
        ...
        # Sports e Leagues
        sport_id = db.insert(
            "sports", {"name": "Test Sport", "slug": "test-sport", "is_active": True}
        )

        league_data = [
            (sport_id, f"League {i}", f"league-{i}", "Test Country", True)
            for i in range(50)
        ]
        db.execute_many(
        ...
        # Markets
        market_id = db.insert(
            "markets", {"name": "Test Market", "slug": "test-market", "is_active": True}
        )
    ...
    def test_concurrent_stress_test(self, threading_database):
        ...
        logging.info(
            f"Stress test: {num_workers} workers, {total_operations} ops, {error_rate:.2f}% errors, {throughput:.2f} ops/sec"
        )
