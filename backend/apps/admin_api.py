...
def create_app(config_overrides=None):
    ...
    app.config["ADMIN_PASSWORD_HASH"] = CONFIG["security"].get(
        "admin_password_hash",
        ...
        def decorator(fn):
            ...
            def wrapper(*args, **kwargs):
                ...
                if not app.jwt_auth.has_permission(user_role, permission):
                    return (
                        jsonify(
                            {"error": "Permissão insuficiente", "required": permission}
                        ),
                        403,
                    )
                return fn(*args, **kwargs)
    ...
    def jwt_login():
        ...
        if username == current_app.config[
            "ADMIN_USERNAME"
        ] and current_app.jwt_auth.verify_password(
            password, current_app.config["ADMIN_PASSWORD_HASH"]
        ):
            access_token = app.jwt_auth.create_token(identity=username, role=ROLE_ADMIN)
            refresh_token = app.jwt_auth.create_refresh_token(
                identity=username, role=ROLE_ADMIN
            )
        ...
        if user and app.jwt_auth.verify_password(password, user["password_hash"]):
            ...
            access_token = app.jwt_auth.create_token(
                identity=user["username"], role=role
            )
            refresh_token = app.jwt_auth.create_refresh_token(
                identity=user["username"], role=role
            )
            ...
            try:
                ...
                logger.warning(
                    f"Falha ao atualizar último login para usuário {username}"
                )

            logger.info(
                f"Login JWT bem-sucedido para usuário '{username}' com role '{role}'"
            )
                    ...
                    jsonify(
                        {
                            ...
                            "permissions": ROLE_PERMISSIONS.get(
                                role, ROLE_PERMISSIONS[ROLE_VIEWER]
                            ),
                        }
                    )
                )
    ...
    def jwt_logout():
        ...
        if use_cookie:
            response = make_response(
                jsonify(
                    {"status": "success", "message": get_text("logout_success", "pt")}
                )
            )
    ...
    def verify_token():
        ...
        if isinstance(identity, str):
            ...
            return (
                jsonify(
                    {"authenticated": False, "message": "Formato de token inválido"}
                ),
                401,
            )
        ...
        claims = get_jwt()
        expiry = (
            datetime.fromtimestamp(claims["exp"]).isoformat()
            if "exp" in claims
            else None
        )
        issued_at = (
            datetime.fromtimestamp(claims["iat"]).isoformat()
            if "iat" in claims
            else None
        )
            ...
            jsonify(
                {
                    ...
                    "issued_at": issued_at,
                    "remaining_seconds": (
                        claims.get("exp", 0) - datetime.now().timestamp()
                        if "exp" in claims
                        else 0
                    ),
                }
    ...
    def admin_dashboard():
        ...
        recent_surebets = db.fetch(
            "SELECT * FROM surebets ORDER BY detected_at DESC LIMIT 10"
        )
    ...
    def operator_dashboard():
        ...
        db = PostgresDatabaseManager()
        surebets_count = db.fetch_one("SELECT COUNT(*) as count FROM surebets")
        active_surebets = db.fetch(
            "SELECT * FROM surebets WHERE status = 'active' ORDER BY detected_at DESC LIMIT 15"
        )
            ...
            jsonify(
                {
                    ...
                    "stats": {
                        "opportunities": (
                            total_opportunities["count"] if total_opportunities else 0
                        ),
                    },
                    "timestamp": datetime.now().isoformat(),
                }
    ...
    def user_profile():
        ...
        user_info["permissions"] = ROLE_PERMISSIONS.get(
            role, ROLE_PERMISSIONS[ROLE_VIEWER]
        )
        ...
        if request.method == "GET":
            ...
            try:
                ...
                if exists:
                    ...
                    logger.info(
                        f"Novo usuário '{user_data['username']}' criado com role '{user_data['role']}'"
                    )
                    return (
                        jsonify(
                            {
                ...
                log_security_event(
                    "VALIDATION_ERROR", f"User creation validation failed: {str(e)}"
                )
    ...
    def delete_user(user_id):
        ...
        return (
            jsonify({"status": "success", "message": "Usuário excluído com sucesso"}),
            200,
        )
        ...
        if request.method == "GET":
            safe_settings = {
                ...
                "active_adapters": list(
                    current_app.bookmaker_integration.list_bookmakers().keys()
                ),
    ...
    def admin_db_overview():
        ...
        try:
            ...
            recent_events = db.fetch(
                "SELECT * FROM events ORDER BY created_at DESC LIMIT 5"
            )
            recent_surebets = db.fetch(
                "SELECT * FROM surebets ORDER BY detected_at DESC LIMIT 5"
            )
    ...
    def admin_insert_bet():
        ...
        try:
            ...
            try:
                ...
                return (
                    jsonify(
                        {"status": get_text("bet_inserted", lang), "bet_id": bet_id}
                    ),
                    200,
                )
                                    ...
                                    if len(selections) >= 2:
                                        ...
                                        if profit >= min_profit:
                                            ...
                                            if (
                                                not search
                                                or search
                                                in opportunity["event"].lower()
                                                or search
                                                in opportunity["market"].lower()
                                            ):
                                                opportunities.append(opportunity)
                        except Exception as adapter_error:
                            logger.warning(
                                f"Erro no adapter {adapter_name}: {adapter_error}"
                            )
                            continue
...
if __name__ == "__main__":
    if not app.config.get("ADMIN_PASSWORD_HASH"):
        logger.critical(
            "ADMIN_PASSWORD_HASH não definido. API não pode iniciar de forma segura."
        )
